...
New column can be added just by defining a new column in the Loblolly data. The date column is defined as a matrix consisting of 1 column, 84 rows, and data of today's date specified by `Sys.Date()`.
```{r}  
Loblolly$date <- matrix(ncol=1, nrow = 84, data = format(Sys.Date()))
head(Loblolly, n = 5)
```   
...
```{r}  
my_list <- list(weekdays(Sys.Date()+0:6, abbreviate = TRUE), month.abb, seq(1:31))
my_list
```  
...
```{r}
my_matrix <- data.frame(Dory = c(1, 2, 3, 4), Edna = c(5, 1, 2, 3), Eva = c(4, 5, 1, 2), Boo = c(3, 4, 5, 1), Violet = c(2, 3, 4, 5))
